...
export async function getEngineRate(
  engine: 'gemini' | 'elevenlabs' | 'chatterbox'
): Promise<number> {
  const FALLBACK = { gemini: 1.2, elevenlabs: 2.5, chatterbox: 0.5 } as const;
  try {
    const { database } = initializeFirebase();
    const snap = await database.ref('settings/pricing').get();
    const v = snap.val() || {};
    const key =
      engine === 'elevenlabs' ? 'elevenLabsNormal'
      : engine === 'chatterbox' ? 'chatterboxNormal'
      : 'studioNormal';
