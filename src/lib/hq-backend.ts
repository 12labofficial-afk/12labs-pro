...
export async function getHQBackendUrl(): Promise<string> {
    ...
    if (!database) return process.env.HQ_BACKEND_URL || 'https://tuf6-oppl.hf.space';
    
    try {
        const snap = await database.ref('admin/config/hq_backend_url').once('value');
        return snap.val() || process.env.HQ_BACKEND_URL || 'https://tuf6-oppl.hf.space';
    } catch (e) {
        return process.env.HQ_BACKEND_URL || 'https://tuf6-oppl.hf.space';
    }
}
