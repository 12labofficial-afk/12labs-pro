...
export async function downloadScriptAsPdf(title: string, rawText: string, filename?: string) {
  if (!rawText) return;
  ...
  for (const lineHtml of htmlLines) {
...
export function downloadScriptAsTxt(title: string, rawText: string, filename?: string) {
  if (!rawText) return;
  const name = filename || `12labs_script_${title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}`;
