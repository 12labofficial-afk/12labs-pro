...
export async function createNewUserProfileOnServer(
  ...
  await requireActionOwner(user.uid);
  ...
  await requireActionOwner(uid);
    try {
        ...
        if (profile?.subscription && 
            ...
            (profile.subscription.weeklyGrantCount || 0) < 4) {
            const now = new Date();
            const nextGrant = new Date(profile.subscription.nextWeeklyGrantDate);
            if (now >= nextGrant) {
...
export async function syncUserSubscriptionInstallments(userId: string): Promise<{ success: boolean; updatedProfile?: UserProfile }> {
    ...
    try {
        const result = await firestore.runTransaction(async (transaction: any) => {
            ...
            if (!sub || (sub.planId !== 'autopay_pro' && sub.planId !== 'test_sub') || (sub.status !== 'active' && sub.status !== 'cancelled') || sub.weeklyGrantCount >= 4) {
                return null;
            }

            const serverNow = new Date();
            let currentNextGrantDate = new Date(sub.nextWeeklyGrantDate);
            ...
            while (serverNow >= currentNextGrantDate && currentWeekCount < 4) {
                ...
                currentWeekCount++; // Moving to next week tier
                
                newHistoryEntries.push({
                    amount: grantAmount,
                    reason: `${planName}: Week ${currentWeekCount} Grant`,
                    timestamp: scheduledTimestamp,
                });

                newNotifications.push({
                    id: `sub-grant-${currentWeekCount}-${Date.now()}`,
                    message: `Weekly Consistency Grant: +${grantAmount.toLocaleString()} Credits added! (Week ${currentWeekCount}/4)`,
                    timestamp: serverNow.toISOString(),
                    read: false,
                    type: 'credits'
                });

                // ADVANCE THE ANCHOR: Strictly move forward by exactly 7 days
                currentNextGrantDate = new Date(currentNextGrantDate.getTime() + 7 * 24 * 60 * 60 * 1000);
            }

            if (totalCreditsToGrant > 0) {
                // If we've hit Week 4, the plan should be deactivated after this grant
                const isPlanFinished = currentWeekCount >= 4;
                ...
                if (isPlanFinished) {
                    ...
                    newNotifications.push({
                        ...
                        message: `Congratulations! Your 28-day Consistency Plan is complete. Your credits will expire 30 days from the original purchase date.`,
                        timestamp: serverNow.toISOString(),
                        read: false,
                        type: 'system'
                    });
        ...
        if (result) {
            ...
            await sendToTelegram(`⚡ <b>Consistency Grants Synchronized</b>\n<b>User:</b> ${result.email}\n<b>Status:</b> ${isDeactivated ? 'Plan Completed (Deactivated)' : `Week ${result.subscription?.weeklyGrantCount}/4`}`);
