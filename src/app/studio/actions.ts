...
import { requireActionUser, requireActionOwner } from '@/lib/server-auth';
    ...
    try {
        ...
        if (editingSettings && editingSettings.enabled !== false && editingSettings.url) {
            const hfRes = await callHFEditingBridge(editingSettings.url, {
                text, voiceId, character, userEmail: sessionUser.email || userEmail, lineId
            });
...
export async function deductFastGenCreditsAction(
    ...
    const multiplier = isChatterbox ? 0.5 : 1.2;
    // Never accept a price from the browser. Pricing is derived server-side.
    const cost = Math.ceil(totalChars * multiplier);
    ...
    try {
        ...
        await database.ref(`creditHistory/${userId}/${historyId}`).set({
            amount: -cost,
            reason: `${isChatterbox ? 'New Studio' : 'Fast Gen'}: ${projectName}`,
            timestamp: new Date().toISOString(),
            projectId
        });

        await logSummaryEvent('creditsSpent', cost);
        await recordIndependenceCashback(userId, cost, `${isChatterbox ? 'New Studio' : 'Fast Gen'}: ${projectName}`);
...
export async function processHighQualityGenerationAndDeductCredits(
  ...
  customCost?: number
    ...
    const cost = Math.ceil(totalChars * 1.2);
    ...
    try {
        ...
        await logSummaryEvent('creditsSpent', cost); 
        await recordIndependenceCashback(userId, cost, `HQ Submission: ${projectName}`, userEmail);
...
export async function regenerateLineWithCreditsAction(userId: string, text: string, voiceId: string): Promise<{ success: boolean; audioDataUri?: string; error?: string; newCredits?: number }> {
    ...
    try {
        ...
        await logSummaryEvent('creditsSpent', cost);
        await recordIndependenceCashback(userId, cost, 'Line Regeneration TTS', userEmail);

        if (database) {
            await database.ref(`creditHistory/${userId}`).push({
                amount: -cost,
                reason: `Regenerate Line (HQ)`,
                timestamp: new Date().toISOString(),
            });
        }
...
export async function createCharacterVoiceReplacementJobAction({
    ...
    try {
        ...
        await logSummaryEvent('creditsSpent', cost);
        await recordIndependenceCashback(userId, cost, `Voice Swap: ${character || 'Bulk Cast'}`, userEmail);
