...
async function handleCreditPurchase(
  ...
  try {
    ...
    const transactionResult = await firestore.runTransaction(async (transaction: any) => {
        ...
        if (isAutopay) {
            creditsToAdd = planSource?.weeklyCredits || 20000;
            planName = `${planSource?.name || 'Consistent Creator'} (Week 1 Grant)`;
        } else if (planSource) {
            creditsToAdd = planSource.credits + bonusFromNotes;
            planName = planSource.name + (bonusFromNotes > 0 ? ' (+Bonus)' : '');
        } else if (notes.credits) {
            creditsToAdd = parseInt(notes.credits, 10) || 0;
            planName = notes.planName || 'AI Credit Pack';
        }
        ...
        if (isAutopay) {
            ...
            userUpdates.subscription = { 
                planId: planSource?.id || 'autopay_pro', 
                status: 'active', 
