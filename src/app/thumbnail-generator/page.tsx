...
export default function ThumbnailGeneratorPage() {
  ...
  const handleExtractYouTube = async (urlToExtract?: string) => {
    ...
    try {
      // Attempt oEmbed to auto-fetch video title
      try {
      ...
      toast({
        ...
        description: 'YouTube title and high-res frame extracted successfully. Write your remix instructions below.',
      });
    } catch (err: any) {
      toast({
        variant: 'destructive',
  ...
  const handleRemixFromDownloader = (imageUrl: string, videoUrl: string, videoTitle: string) => {
    setYtLink(videoUrl);
    setTitle(videoTitle || 'Remix Masterpiece');
    setExtractedYtImage(imageUrl);
    setShowPromptInput(true);
    setActiveTab('generate');
    toast({
      title: '🎨 Remix Active!',
      description: 'Loaded original video title & image reference. Select style and generate your new thumbnail.',
    });
  };
...
5. ZERO WATERMARKS: Strictly NO developer logos, AI badges, or corner watermark stamps anywhere in the image.`.trim();
  ...
  return (
    <div className="min-h-screen bg-[#212121] text-[#E2E8F0] pb-24 selection:bg-[#9C27B0]/30 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-10 relative z-10">
        ...
        <div className="flex justify-center mb-8">
          <div className="grid grid-cols-2 p-1 bg-[#181818] rounded-2xl border border-[#3a3a3a] w-full max-w-md">
            <button
              ...
              className={cn(
                ...
                activeTab === 'generate'
                  ...
                  : "text-slate-400 hover:text-white hover:bg-[#222]/50"
              )}
            >
        ...
        {activeTab === 'generate' ? (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in duration-300">
            ...
            <div className="p-5 sm:p-6 bg-[#181818] rounded-2xl border border-[#3a3a3a] shadow-2xl space-y-5">
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-300 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-slate-400 font-normal">Reference & auto-fill</span>
                </label>
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    className="text-xs h-11 bg-[#2e2e2e] border-[#3a3a3a] text-white placeholder:text-slate-500 rounded-xl focus:border-[#9C27B0]"
                  />
                  <Button 
                    type="button" 
                    variant="secondary" 
                    size="sm" 
                    className="h-11 px-4 text-xs font-semibold shrink-0 bg-[#3a3a3a] hover:bg-[#4a4a4a] text-white rounded-xl"
                ...
                {extractedYtImage && (
                  <div className="relative mt-2 rounded-xl overflow-hidden border border-[#3a3a3a] aspect-[16/9] bg-black/60 group">
                    <img 
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-300 flex items-center justify-between">
                  <span>2. Video Title / Topic (Optional)</span>
                  <span className="text-[10px] text-[#9C27B0] font-medium">Core Subject</span>
                </label>
                <Input
                  placeholder="e.g. Top 10 Hidden Mysteries, GTA 6 Secret Mission..."
                  value={title}
                ...
                {(showPromptInput || extractedYtImage) ? (
                  <div className="space-y-2">
                    <Textarea
                      ...
                      rows={4}
                      className="text-xs bg-[#2e2e2e] border-[#3a3a3a] text-white placeholder:text-slate-500 rounded-xl leading-relaxed resize-none focus:border-[#9C27B0]"
                    />
                  </div>
                ) : (
                  <p className="text-[11px] text-slate-400 bg-[#222222] p-2.5 rounded-xl border border-[#3a3a3a] leading-normal">
                    {prompt.trim() ? (
                      <span className="text-purple-300 truncate block">Prompt: {prompt}</span>
                    ) : (
              ...
              <div className="space-y-1.5">
                ...
                <Select value={selectedStyle} onValueChange={setSelectedStyle}>
                  <SelectTrigger className="text-xs h-11 bg-[#2e2e2e] border-[#3a3a3a] text-white rounded-xl focus:ring-[#9C27B0]">
                    <SelectValue placeholder="Select Style" />
                  </SelectTrigger>
                  <SelectContent className="bg-[#181818] border-[#3a3a3a] text-white">
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        aspectRatio === key 
                          ...
                          : "border-[#3a3a3a] bg-[#2e2e2e]/60 text-slate-400 hover:border-slate-500 hover:bg-[#2e2e2e]"
                      )}
                    >
                      <span className="font-bold text-white">{config.label}</span>
                      <span className="text-[10px] text-slate-400 mt-0.5">{config.sub}</span>
                    </button>
                  ))}
                </div>
              </div>
          ...
          <div className="lg:col-span-6 space-y-4">
            ...
            <div className="p-5 sm:p-6 bg-[#181818] rounded-2xl border border-[#3a3a3a] shadow-2xl min-h-[460px] flex flex-col justify-between">
              
              {/* Header of Preview Box */}
              <div className="flex items-center justify-between border-b border-[#3a3a3a] pb-3.5 mb-4">
              ...
              <div className="flex-1 flex flex-col items-center justify-center">
                ...
                {isJobProcessing && (
                  <div className="text-center space-y-4 py-12">
                    ...
                    <div className="space-y-1">
                      <h3 className="text-sm font-bold text-white">Synthesizing High-Resolution Thumbnail</h3>
                ...
                {isJobReady && (activeJob?.imageUrl || (activeJob as any)?.link || (activeJob as any)?.url || (activeJob as any)?.image) && (
                  <div className="w-full space-y-4 animate-in fade-in zoom-in-95 duration-300">
                    <div className="relative w-full rounded-xl overflow-hidden border border-[#3a3a3a] shadow-2xl bg-black group">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
                      <div className="space-y-0.5">
                        <p className="text-xs font-bold text-white truncate max-w-[260px]">{activeJob.title}</p>
                        <p className="text-[10px] text-slate-400">{activeJob.aspectRatio} • {activeJob.style}</p>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <Button
                          variant="outline"
                          size="sm"
                          className="h-9 text-xs gap-1.5 rounded-xl border-[#3a3a3a] bg-[#2e2e2e] hover:bg-[#3a3a3a] text-slate-200"
                ...
                {!isJobProcessing && !isJobReady && (
                  <div className="text-center space-y-3 py-16 text-slate-500">
                    <div className="mx-auto w-14 h-14 rounded-2xl bg-[#2e2e2e]/80 border border-[#3a3a3a] flex items-center justify-center text-slate-400">
                      <ImageIcon className="h-7 w-7" />
                    </div>
                    <div className="space-y-1">
                      <p className="text-xs font-semibold text-slate-300">Ready to Generate</p>
              ...
              {/* Bottom Info */}
              <div className="pt-4 border-t border-[#3a3a3a] mt-4 flex items-center justify-between text-[11px] text-slate-400">
            ...
            {recentJobs.length > 0 && (
              <div className="p-4 bg-[#181818] rounded-2xl border border-[#3a3a3a] space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {recentJobs.map((job) => {
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          "relative rounded-xl overflow-hidden border p-2 bg-[#2e2e2e]/60 cursor-pointer transition-all hover:bg-[#2e2e2e] group",
                          activeJob?.mappingId === job.mappingId 
                            ? "border-[#9C27B0] ring-2 ring-[#9C27B0]/30" 
                            : "border-[#3a3a3a] hover:border-slate-400"
                        )}
                      >
          ...
          <div className="space-y-8 max-w-4xl mx-auto animate-in fade-in duration-300">
            <Card className="border border-border/60 shadow-2xl shadow-primary/5 overflow-hidden rounded-[2.5rem] bg-card/90 backdrop-blur-3xl">
              <CardHeader className="text-center pt-10 pb-6">
                ...
                <h2 className="text-2xl sm:text-3xl font-black tracking-tighter uppercase leading-tight text-white">
                  YouTube Thumbnail <span className="text-[#9C27B0]">Extractor</span>
                </h2>
                <p className="text-xs sm:text-sm max-w-md mx-auto mt-2 font-bold text-slate-400 leading-relaxed uppercase tracking-wider">
              ...
              <CardContent className="max-w-2xl mx-auto pb-12">
                <div className="flex flex-col sm:flex-row items-center gap-3">
                  <div className="relative flex-grow w-full group">
                    <Link2 className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-500 transition-colors group-focus-within:text-[#9C27B0]" />
                    <Input
                      type="text"
                      ...
                      className="pl-12 h-14 text-xs sm:text-sm bg-muted/40 border-border/60 text-white placeholder:text-slate-500 rounded-xl focus-visible:ring-[#9C27B0] focus-visible:ring-offset-0 focus:border-[#9C27B0]"
            ...
            {isFetchingDownloader && (
              <div className="text-center py-16 animate-in fade-in">
                ...
                <p className="text-sm font-black text-slate-400 animate-pulse uppercase tracking-wider">Analyzing YouTube server frames...</p>
              </div>
            )}
            ...
            {downloaderThumbnails.length > 0 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-500">
                <div className="flex flex-col gap-1 items-center text-center">
                  <h3 className="text-lg sm:text-xl font-black uppercase tracking-tight flex items-center gap-2 text-white">
                    <ImageIcon className="text-[#9C27B0] h-5 w-5" /> 
                    <span>Available Resolutions</span>
                  </h3>
                  {downloaderVideoTitle && (
                    <p className="text-slate-400 font-bold max-w-2xl text-xs sm:text-sm italic opacity-80 px-4">&quot;{downloaderVideoTitle}&quot;</p>
                  )}
                </div>
                    ...
                    <Card key={thumb.quality} className={cn(
                      ...
                      <CardContent className="p-0 relative">
                        ...
                        <div className="p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between bg-card gap-3 border-t border-border/60">
                          <div>
                            <h4 className="font-black text-base sm:text-lg tracking-tight uppercase leading-none text-white">{thumb.quality}</h4>
                            <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-1.5">Resolution {thumb.resolution}</p>
                          </div>
                          <div className="flex gap-2 w-full sm:w-auto">
                            <Button 
                              size="sm" 
                              variant="outline"
                              className="rounded-xl px-4 h-10 text-[10px] uppercase font-bold tracking-wider border-border/60 bg-muted/40 hover:bg-muted/60 text-[#E2E8F0] gap-1.5 w-1/2 sm:w-auto" 
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4">
              <div className="p-5 bg-card/90 backdrop-blur-3xl rounded-2xl border border-border/60">
                ...
                <p className="text-[11px] text-slate-400">Put any YouTube video link inside the box and extract immediate server images.</p>
              </div>
              <div className="p-5 bg-card/90 backdrop-blur-3xl rounded-2xl border border-border/60">
                <p className="font-bold text-[#9C27B0] text-xs sm:text-sm uppercase mb-1">2. Download directly</p>
                <p className="text-[11px] text-slate-400">Save original high-resolution thumbnail images to your device with one single click.</p>
              </div>
              <div className="p-5 bg-card/90 backdrop-blur-3xl rounded-2xl border border-border/60">
                <p className="font-bold text-[#9C27B0] text-xs sm:text-sm uppercase mb-1">3. Remix in Studio</p>
                <p className="text-[11px] text-slate-400">Want an improved version? Hit remix to generate an outstanding custom variant using AI.</p>
              </div>
            </div>
          </div>
        )}
