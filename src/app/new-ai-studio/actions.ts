    ...
    try {
        ...
        if (editingSettings && editingSettings.enabled !== false && editingSettings.url) {
            const hfRes = await callHFEditingBridge(editingSettings.url, {
                text, refAudioUrl, language, userId: sessionUser.uid, userEmail: sessionUser.email || userEmail, character, lineId
            });
...
export async function deductNewStudioCreditsAction(
    ...
    try {
        ...
        if (remainingCost > 0) {
            ...
            await recordIndependenceCashback(userId, remainingCost, `New AI Studio: ${projectName}`);
        }
