...
export async function getAvailablePromoCodes(): Promise<{ success: boolean; codes?: PromoCode[]; message: string }> {
    const { firestore } = initializeFirebase();
    try {
...
export async function searchPromoCode(codeId: string): Promise<{ success: boolean; code?: PromoCode; message: string }> {
  if (!codeId) {
...
export async function deletePromoCode(code: string): Promise<{ success: boolean; message: string }> {
  if (!code) {
    return { success: false, message: 'Code is required.' };
  }
  const { firestore } = initializeFirebase();
...
export async function getAffiliateData(): Promise<{ success: boolean; data?: { codes: AffiliateCode[] } , message: string }> {
    const { database } = initializeFirebase();
    try {
