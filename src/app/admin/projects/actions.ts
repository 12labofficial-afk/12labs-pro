...
/**
 * Pending submissions live in RTDB (the seller upload flow intentionally keeps
 * the full downloadable asset out of Firestore). Read them server-side so the
 * moderator queue does not depend on the browser's RTDB rules or admin claims.
 */
export async function getPendingProducts(): Promise<{ success: boolean; products?: Product[]; message: string; }> {
    const { database } = initializeFirebase();
    try {
        const snapshot = await database.ref('pendingProducts').get();
        if (!snapshot.exists()) {
            return { success: true, products: [], message: 'No pending products found.' };
        }

        const products = Object.values(snapshot.val()).map((product: any) => ({
            ...product,
            previews: product.previews && !Array.isArray(product.previews) ? Object.values(product.previews) : product.previews,
            downloadableFiles: product.downloadableFiles && !Array.isArray(product.downloadableFiles) ? Object.values(product.downloadableFiles) : product.downloadableFiles,
            scriptPreview: product.scriptPreview && !Array.isArray(product.scriptPreview) ? Object.values(product.scriptPreview) : product.scriptPreview,
        })) as Product[];

        return { success: true, products, message: 'Pending products fetched successfully.' };
    } catch (error: any) {
        console.error('Error fetching pending products:', error);
        return { success: false, products: [], message: error.message || 'Failed to fetch pending products.' };
    }
}
