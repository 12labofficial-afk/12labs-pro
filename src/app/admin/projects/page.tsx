...
import { approveProduct, rejectProduct, getPendingProducts, getProductsByStatus, getCompleteProduct } from './actions';
...
export default function AdminProductsPage() {
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        getPendingProducts().then((result) => {
            if (cancelled) return;
            const productsArray = result.success ? (result.products || []) : [];
            setPendingProducts(productsArray.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()));
            setIsPendingLoading(false);
            if (!result.success) {
                toast({ variant: 'destructive', title: 'Queue unavailable', description: result.message });
            }
        });
    ...
    useEffect(() => {
        ...
        const unsubscribe = onValue(ref(database, 'storeProducts'), (snapshot) => {
