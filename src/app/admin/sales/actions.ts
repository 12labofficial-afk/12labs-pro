...
export async function getSellersList(): Promise<{ success: boolean; data?: SellerSummary[]; message: string }> {
  try {
...
export async function getSellerProducts(sellerId: string): Promise<{ success: boolean; products?: Product[]; message: string; }> {
  try {
...
export async function deleteProductAdminAction(productId: string): Promise<{ success: boolean; message: string; }> {
  const { firestore, database } = initializeFirebase();
