    ...
    try {
        ...
        if (userDoc.exists) {
            ...
            if (data?.subscription && 
                ...
                (data.subscription.weeklyGrantCount || 0) < 4) {
                const now = new Date();
...
export async function banUser(userId: string, adminEmail: string): Promise<{ success: boolean; error?: string }> {
  try {
    const { auth, firestore } = initializeFirebase();
    if (!firestore) return { success: false, error: 'Database instance unavailable.' };
...
export async function suspendUser(userId: string, days: number, adminEmail: string): Promise<{ success: boolean; error?: string }> {
  try {
    const { auth, firestore } = initializeFirebase();
    if (!firestore) return { success: false, error: 'Database instance unavailable.' };
    if (auth) await auth.updateUser(userId, { disabled: true }).catch((e: any) => console.warn("Auth disable warning:", e));
...
export async function reactivateUser(userId: string, adminEmail: string): Promise<{ success: boolean; error?: string }> {
  try {
    const { auth, firestore } = initializeFirebase();
    if (!firestore) return { success: false, error: 'Database instance unavailable.' };
                    ...
                    if (rtdbData && typeof rtdbData === 'object') {
                        for (const key of Object.keys(rtdbData)) {
                            ...
                            if (!stillExists) {
                                ...
                                console.log(`Deleted RTDB credit history entry: ${key}`);
                            }
                        }
                    }
                }
                ...
                if (doc.id !== 'history_log') {
                    ...
                    if (data && data.timestamp && data.reason) {
                        ...
                        if (!stillExists) {
                            ...
                            console.log(`Deleted Firestore individual credit history doc: ${doc.id}`);
                        }
                    }
                }
            }
