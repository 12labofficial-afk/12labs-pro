...
function DailyFreeScriptLimitSettings() {
    ...
    useEffect(() => {
        ...
        const unsubLimit = onValue(ref(db, 'settings/app/dailyFreeScriptLimit'), (snapshot) => {
            if (snapshot.exists()) {
                setLimit(Number(snapshot.val()) || 40);
            }
        });

        const unsubCount = onValue(ref(db, `dailyFreeScriptGenerations/${today}/count`), (snapshot) => {
            if (snapshot.exists()) {
...
function HqBackendSettings() {
    ...
    useEffect(() => {
        const { database: db } = initializeFirebase();
        if (!db) return;
        const unsubscribe = onValue(ref(db, 'admin/config/hq_backend_url'), (snapshot) => {
            if (snapshot.exists()) {
                setUrl(snapshot.val() || '');
            }
        });
            ...
            <CardContent className="p-6 space-y-6">
                <div className="space-y-2">
                    ...
                    <Input 
                        ...
                        placeholder="https://tuf6-oppl.hf.space" 
...
function EditingHfBackendSettings() {
    ...
    useEffect(() => {
        const { database: db } = initializeFirebase();
        if (!db) return;
        const unsubscribe = onValue(ref(db, 'settings/editingHfBackend'), (snapshot) => {
            if (snapshot.exists()) {
                const data = snapshot.val();
                setUrl(data.url || '');
...
function AnalysisExecutionModeSettings() {
    ...
    useEffect(() => {
        const { database: db } = initializeFirebase();
        if (!db) return;
        const unsubscribe = onValue(ref(db, 'settings/analysisExecutionMode'), (snapshot) => {
            if (snapshot.exists()) {
...
export default function AdminPage() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        let todayJoined = 0;
        try {
          const usersCount = await getCountFromServer(collection(fs, 'users'));
          uCount = usersCount?.data()?.count || 0;
        } catch {}
        try {
          const projectsCount = await getCountFromServer(collection(fs, 'projects'));
          pCount = projectsCount?.data()?.count || 0;
        } catch {}
        try {
  ...
  useEffect(() => {
    const { database: db } = initializeFirebase();
    if (!db) return;
    const unsubToolSettings = onValue(ref(db, 'toolSettings'), (snap) => {
      setToolSettings(snap.val() || {});
      setToolSettingsLoading(false);
    });
    const unsubWatermark = onValue(ref(db, 'settings/app/musicWatermarkUrl'), (snap) => { setMusicWatermark(snap.val() || ''); });
  ...
  const adminTools = [
    ...
    { id: 'voice-india', label: 'Voice India', icon: <Globe className="h-3 w-3 text-orange-500" /> },
    ...
    { id: 'chatbot', label: 'AI Chatbot', icon: <BrainCircuit className="h-3 w-3 text-blue-400" /> },
  ...
  return (
    <div className="space-y-6 pb-20 max-w-7xl mx-auto w-full px-1">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
        {[
          { href: '/admin/users', label: 'Users', detail: 'Manage users', icon: <Users className="h-4 w-4" /> },
          { href: '/admin/payments', label: 'Payments', detail: 'Review activity', icon: <IndianRupee className="h-4 w-4" /> },
          { href: '/admin/chat', label: 'Live Chat', detail: 'Reply to users', icon: <MessageCircle className="h-4 w-4" /> },
          { href: '/admin/projects', label: 'Moderator', detail: 'Review products', icon: <Package className="h-4 w-4" /> },
          { href: '/admin/withdrawals', label: 'Withdrawals', detail: 'Seller requests', icon: <Banknote className="h-4 w-4" /> },
        ].map((item) => (
          <Link key={item.href} href={item.href} className="group">
            <Card className="h-full rounded-xl border-primary/10 shadow-sm transition-all hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md">
              <CardContent className="flex items-center gap-2.5 p-3">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary transition-transform group-hover:scale-105">{item.icon}</div>
                <div className="min-w-0">
                  <p className="truncate text-[10px] font-black uppercase tracking-wider">{item.label}</p>
                  <p className="mt-0.5 truncate text-[8px] font-bold uppercase tracking-wide text-muted-foreground">{item.detail}</p>
                </div>
              </CardContent>
            </Card>
          </Link>
