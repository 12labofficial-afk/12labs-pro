...
export default function SellerClientLayout({ children }: { children: React.ReactNode; }) {
  ...
  const hasSellerAccess = Boolean(user?.isSeller || user?.role === 'admin');

  // Keep protected seller screens out of the UI while the redirect runs.
  // Previously a normal user could briefly see the seller dashboard (and
  // sometimes trigger its Firestore queries) before router.push completed.
  if (!authLoading && (!user || (isSellerDashboardPage && !hasSellerAccess))) {
    return (
        ...
        <div className="flex flex-1 items-center justify-center p-6">
          <div className="flex flex-col items-center gap-4 text-center text-muted-foreground">
            ...
            <p className="font-medium">
              {!user ? 'Taking you to sign in...' : 'Checking seller access...'}
            </p>
          </div>
        </div>
      </div>
  ...
  const showSidebar = hasSellerAccess;
