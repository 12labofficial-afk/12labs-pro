...
interface ProStudioContextType {
  ...
  // 🎙️ Which voice engine the NEXT generation will use. "gemini" is the
  // existing default. Set to "elevenlabs" + a voice_id (from the 11Labs
  // Voice Library picker) to route the queue item to the new engine —
  // see actions.ts / pro_studio_elevenlabs.py.
  engine: 'gemini' | 'elevenlabs';
  setEngine: (e: 'gemini' | 'elevenlabs') => void;
...
export function ProStudioProvider({ children }: { children: ReactNode }) {
  ...
  // "gemini" projects queue in pro_projects, "elevenlabs" ones in their own
  // 11_projects RTDB node (server-files/11.py) — see rtdbProgressNode below.
  // The Firestore DOC, however, always lives in 'pro_projects' regardless
  // of engine (see pro-studio/actions.ts) — that's the one collection
  // /history actually reads, so reading it from '11_projects' here would
  // just show nothing once the project completes.
  const rtdbProgressNode = engine === 'elevenlabs' ? '11_projects' : 'pro_projects';
  useEffect(() => {
    if (!database || !hqProjectId || hqProjectId === 'PENDING') return;
    const progressRef = ref(database, `${rtdbProgressNode}/${hqProjectId}`);
    ...
    return () => unsubscribe();
  }, [database, hqProjectId, rtdbProgressNode]);
  ...
  const analyzeScript = async () => {
      ...
      setTimeout(() => {
        try {
          ...
          if (extracted.length > 0) {
            ...
            // 🔴 FIX: `proVoices[0].id` crashed this whole analysis if the
            // Gemini pro-voice list hadn't finished loading yet (race
            // condition) — leaving characters undetected entirely rather
            // than just missing a default voice. Guarded to fall back to
            // an empty string (user picks manually) instead of throwing.
            // Note: pro-studio's 11Labs path doesn't use per-character
            // `voice` at all — it uses a single project-wide
            // `elevenLabsVoiceId` (see `engine`/`elevenLabsVoiceId` state
            // above), so this default only ever matters for Gemini.
    ...
    try {
        ...
        // 🔴 Same crash-guard as above — see that comment.
  ...
  const applyExternalAnalysis = (rawInput: any): boolean => {
    try {
      ...
      // 🔴 FIX: same crash-guard pattern as elsewhere in this file — if
      // `proVoices` was empty (not loaded yet), `proVoices[0]` was
      // `undefined` and `.id` threw, silently leaving voices unassigned.
      const getRandomVoiceId = (aiGender: string) => {
        if (!proVoices || proVoices.length === 0) return '';
  ...
  const handleGeneration = async () => {
    ...
    if (engine === 'elevenlabs' && !elevenLabsVoiceId) {
        toast({ variant: 'destructive', title: 'Pick a voice', description: 'Select an 11Labs voice before generating.' });
        return;
    }
  ...
  const clearStudioState = async () => {
      ...
      setEngine('gemini'); setElevenLabsVoiceId(null);
