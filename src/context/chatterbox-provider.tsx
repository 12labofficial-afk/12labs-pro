...
export function ChatterboxProvider({ children }: { children: ReactNode }) {
    ...
    useEffect(() => {
        if (!database) return;
        onValue(ref(database, 'clone_studio'), (snap) => { 
            if (snap.exists()) { 
        ...
        if (activeUid) onValue(ref(database, `users/${activeUid}/library_selections`), (snap) => { 
    ...
    const finalizeMaster = async () => {
        ...
        } catch (e: any) { toast({ variant: 'destructive', title: 'Mastering Failed', description: e.message }); }
