...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (targetUser && targetUser.subscription && (targetUser.subscription.planId === 'autopay_pro' || targetUser.subscription.planId === 'test_sub')) {
        ...
        if (now >= nextGrant && (targetUser.subscription.weeklyGrantCount || 0) < 4) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onIdTokenChanged(auth, async (firebaseUser) => {
      ...
      setLoading(true);
      // Keep a server-only session cookie in sync so protected routes cannot
      // be opened by merely changing client-side role state.
      try {
        ...
        console.warn('Server session sync failed:', sessionError);
      }
      ...
      try {
        ...
        if (raw) {
          ...
          if (parsed && (parsed.uid === firebaseUser.uid || parsed.id === firebaseUser.uid)) {
            ...
            setUser({ ...firebaseUser, ...cachedProfile } as User);
          }
        }
      } catch {
