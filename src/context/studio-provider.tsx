...
interface StudioContextType {
  ...
  applyExternalAnalysis: (rawInput: any, isClaude?: boolean) => boolean;
...
export function StudioProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!database) return;
    const unsubMode = onValue(ref(database, 'settings/analysisExecutionMode'), (snap) => {
  ...
  useEffect(() => {
    if (!database) return;
    onValue(ref(database, 'toolSettings/premium-only'), (snapshot) => { 
  ...
  const updateGeneratedLine = (lineId: string, updates: Partial<GeneratedLine>) => {
      setGeneratedLines(prev => {
          ...
          const discountedRate = pricingRef.current.discounted || 1.2;
          setScriptAnalysis(sa => sa ? {
              ...sa,
              characterCount: totalChars,
              dialogueCount: next.length,
              cost: sa.isDiscounted ? Math.ceil(totalChars * discountedRate) : Math.ceil(totalChars * normalRate),
    ...
    try {
        if (generationMode === 'high-quality') {
            ...
            if (!isPaid) {
                ...
                const normalRate = pricingRef.current.normal || 1.2;
                const discountedRate = pricingRef.current.discounted || 1.2;
                const fastGenCost = scriptAnalysis.isDiscounted ? Math.ceil(actualChars * discountedRate) : Math.ceil(actualChars * normalRate);
  ...
  const applyExternalAnalysis = (rawInput: any, isClaude: boolean = false): boolean => {
    try {
      let rawLines: any[] = [];
      let rawChars: any[] = [];
      ...
      const normalRate = pricingRef.current.normal || 1.2;
      const discountedRate = pricingRef.current.discounted || (normalRate * 0.8);
      
      // The discount is exclusively for scripts verified by the Claude
      // Formatter flow. Analysis returned by the admin-triggered third-party
      // RTDB worker must always use the normal rate.
      const isDiscounted = isClaude;
      const activeDiscountedRate = isClaude ? discountedRate : normalRate;
      const originalCost = Math.ceil(totalDialogueChars * normalRate);
      const discountedCost = Math.ceil(totalDialogueChars * activeDiscountedRate);
      const discountPercentage = isClaude ? (Math.round((1 - activeDiscountedRate / normalRate) * 100) || 20) : 0;
      ...
      setHqSubmissionId(`HQ_${generateShortId()}_${timestamp}`);
      logSummaryEvent(isClaude ? 'discountScriptAnalysis' : 'normalScriptAnalysis').catch(() => null);
      ...
      if (isClaude) {
        toast({
          ...
          description: `Claude formatting verified! Script imported successfully with 20% reduced credit cost (${discountedCost} credits).`
        });
      }
      return true;
    } catch (e: any) {
