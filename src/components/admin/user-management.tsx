...
function UserUnifiedViewDialog({ 
    ...
    const getViewingScriptText = () => {
        if (!viewingProject) return '';
        if (typeof viewingProject.script === 'string' && viewingProject.script) return viewingProject.script;
        if (typeof viewingProject.generationParams === 'string' && viewingProject.generationParams) return viewingProject.generationParams;
        if (typeof (viewingProject.generationParams as any)?.prompt === 'string') return (viewingProject.generationParams as any).prompt;
        return viewingProject.projectName || '';
    };

    const handleDownloadScriptTxt = () => {
        const text = getViewingScriptText();
        if (!text) {
