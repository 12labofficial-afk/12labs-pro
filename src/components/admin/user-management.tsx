...
function UserUnifiedViewDialog({ 
        ...
        try {
            ...
            switch (confirmAction.type) {
                case 'role': 
                    ...
                    await updateUserRole(user.uid, user.email, updatedProfile.role); 
                    break;
                case 'seller': 
                    updatedProfile.isSeller = !user.isSeller; 
                    await toggleSellerStatus(user.uid, user.email, user.isSeller!); 
                    break;
                case 'sponsor': 
                    updatedProfile.isSponsor = !user.isSponsor; 
                    await toggleSponsorStatus(user.uid, user.email, user.isSponsor!); 
                    break;
                case 'ban': 
                    updatedProfile.status = 'banned'; 
                    await banUser(user.uid, adminUser?.email || ''); 
                    break;
                case 'suspend':
                    ...
                    await suspendUser(user.uid, days, adminUser?.email || '');
                    break;
                case 'reactivate': 
                    updatedProfile.status = 'active'; 
                    delete updatedProfile.suspensionEndDate; 
                    await reactivateUser(user.uid, adminUser?.email || ''); 
                    break;
                case 'cancel_sub': {
                    const res = await updateUserSubscription(user.uid, null, adminUser?.email || '');
    ...
    const handleGrantAutopay = async () => {
        ...
        setIsUpdating(true);
        const res = manuallyGrantAutopayAction(user.uid, adminUser.email);
...
export function UserManagement() {
  ...
  const handleAdjustCredits = async () => {
    ...
    try {
        ...
        const result = await adjustUserCredits(selectedProfile.uid, selectedProfile.credits, finalBalance, currentUser.email, creditReason);
        if (result.success) {
