...
export default function ProductView({ initialProduct, initialSeller }: ProductViewProps) {
    ...
    useEffect(() => {
        ...
        return onValue(pricingRef, (snap) => {
            if (snap.exists()) {
    ...
    useEffect(() => {
        if (!database) return;
        onValue(ref(database, 'settings/landingPage'), (snap) => {
            if (snap.exists()) {
                const data = snap.val();
    ...
    const handleDownloadLicense = async () => {
        ...
        try {
            const masterLogoBuffer = logos.master ? await (async () => { try { const res = await fetch(logos.master); return await res.arrayBuffer(); } catch(e) { return null; } })() : null;
            const watermarkLogoBuffer = logos.watermark ? await (async () => { try { const res = await fetch(logos.watermark); return await res.arrayBuffer(); } catch(e) { return null; } })() : null;
                        ...
                        <div className="space-y-6 pt-12">
                            ...
                            {isScript && (scriptPreviewText.length > 0 || isFetchingPreview) && (
                                <div className="space-y-4 pt-6">
                                    ...
                                    <div className="border-2 border-primary/10 rounded-[2rem] p-6 sm:p-10 bg-muted/20 font-mono text-sm leading-relaxed shadow-inner relative">
                                        {isFetchingPreview ? (
