...
export function GenerationSettings() {
    ...
    // Cost Breakdown & Discount logic
    const originalCost = scriptAnalysis?.originalCost || Math.ceil(totalChars * 1.2);
    // Only the explicit Claude Formatter verification can activate the offer.
    // Do not infer a discount from a rate mismatch returned by another node.
    const isDiscounted = scriptAnalysis?.isDiscounted === true;
    const savedCredits = Math.max(0, originalCost - currentCost);
    const discountPercent = isDiscounted ? (scriptAnalysis?.discountPercentage || 20) : 0;
        ...
        <Card className="border-border/60 shadow-2xl bg-card/90 dark:bg-white/[0.02] backdrop-blur-3xl overflow-hidden rounded-[2rem]">
                        ...
                        <div className="flex items-center justify-between px-1">
                            ...
                            {isDiscounted ? (
                                <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-500">
                                    ...
                                    <span className="text-[9px] font-black uppercase">20% DISCOUNT APPLIED</span>
                                </div>
                            ) : (
                        ...
                        {isDiscounted ? (
                            <div className="space-y-1.5 px-1 pt-1">
                                <div className="flex items-baseline gap-2.5 flex-wrap">
                                    <span className="font-mono text-base font-bold text-muted-foreground line-through decoration-destructive decoration-2">
                                    ...
                                    <Badge className="bg-emerald-600 text-white text-[9px] font-black px-2 h-5 rounded-md">
                                        -20% OFF
                                    </Badge>
                                </div>
                                <p className="text-[11px] font-bold text-emerald-500">
                                    🎉 You saved <strong className="font-mono">{savedCredits.toLocaleString()}</strong> credits with Claude Formatter!
                                </p>
                            </div>
                        ) : (
            ...
            <CardFooter className="p-8 pt-0 flex flex-col gap-4">
                <Button 
                    ...
                    disabled={isFinalizing || !isReady || (isFastGenDisabled && generationMode === 'fast')} 
