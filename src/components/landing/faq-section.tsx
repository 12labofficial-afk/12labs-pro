...
const faqItems = [
  ...
  },
  {
    ...
    answer: "We offer a comprehensive suite: AI Voice Studio (both Fast and Pro HQ modes), Script Generator, Chatbot, PDF Tools, YouTube SEO Kit, Thumbnail Generator, Sound Search, and YouTube Transcript Downloader. We're constantly adding new tools to help creators streamline their workflow."
  },
  {
    ...
    <section className="relative z-10 w-full py-20 md:py-32 bg-background border-t border-border/50">
      <div className="container px-4 md:px-6 max-w-4xl mx-auto">
        ...
        <motion.div
          ...
          className="bg-card/80 dark:bg-zinc-900/60 backdrop-blur-xl rounded-3xl border border-border/80 dark:border-white/10 p-4 sm:p-8 shadow-xl transition-shadow duration-500 hover:shadow-2xl"
        >
          <Accordion type="single" collapsible className="w-full">
            {faqItems.map((faq, index) => (
              <AccordionItem key={index} value={`item-${index}`} className="faq-glass-item border-b border-border/40 last:border-0 px-3 sm:px-5 mb-3 last:mb-0 rounded-2xl overflow-hidden">
                <AccordionTrigger className="text-base sm:text-lg font-bold text-left text-foreground hover:text-primary transition-colors py-5 px-2">
                  {faq.question}
                </AccordionTrigger>
                <AccordionContent className="faq-answer text-sm sm:text-base text-muted-foreground leading-relaxed pb-5 font-medium px-2">
                  {faq.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
