...
export function GenerationSettings() {
    ...
    // Gemini engine needs every character assigned a voice (CharacterAssignments).
    // 11Labs engine needs one library voice selected instead — see ElevenLabsVoicePicker.
    const isReady = projectName.trim().length > 0 &&
        (engine === 'elevenlabs' ? !!elevenLabsVoiceId : characters.every(c => c.voice));
