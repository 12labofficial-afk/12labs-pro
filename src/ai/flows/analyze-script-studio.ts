 ...
 */
async function pollAnalysisResult(userId: string, mappingId: string, timeoutMs: number = 120000): Promise<any> {
...
function normalizeAndChunkLines(rawLines: { character: string; text: string; emotion?: string }[]): { character: string; text: string; emotion: string }[] {
    ...
    rawLines.forEach((line) => {
        ...
        if (['वक्ता', 'कथावाचक', 'नैरेटर', 'speaker', 'background', 'narration'].includes(currentChar.toLowerCase())) {
